...
| Presentation | `cli` | A bare `percept` prints the start screen - how to record, then the project's maps whole - the one text a coding client's session also opens on; `docs/cli.md` is the design the commands are moving toward. `percept events search`, `show`, `percept maps` - the log and its maps without the TUI; `ask` and `code`, the TUI, under `lab`. `hook <client>` records a coding client's turn from the hook JSON on stdin; `init <client>` writes the shipped schemas to `.percept/schemas` and the client's config to call the hook. Headless, a call the policy would ask about is declined unless `ask --yes`. |
| Presentation | `server` | `percept web` - serves the embedded page and the cuts it reads. `GET /api/events` is this project's log filtered by the same query `events search` builds, and `GET /api/events/{id}` one event whole. The cut is the view's, not a general query: a `tool.resulted` is never a row, so it never appears among the matches, and one whose call is in the cut comes back under `carried` for the row that folds it. `GET /api/projects` and `GET /api/maps/{id}` answer for more than this project: the first for every root the log carries, with each one's maps, their node count, and what each gained since the last session before this view opened; the second for the map `id` names, folded from that project's own schemas and cut to a `Selection` - the nodes that head it alone with no `around`, or `around` a node to `depth` - with `Fragment`'s counts saying what the cut left out. Every other path serves the page, which routes itself; an unknown path under `/api` is a 404. Read-only for now; the review queue's write moves back with the Attention lens. |
...
| Infrastructure | `mapstore` | Loads the schemas from `.percept/schemas/*.toml`, holds the templates `init` writes there, and folds a log-backed cognitive map (`LogMaps`, the `core::MapReader`), revises it, and gives it an external form: `encode_*` to JSON lines, `markdown`/`catalogue` to the text `maps show`/`maps list` print - read live, never written to a file. |
