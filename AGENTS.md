...
- `Model`: "a reply given messages", domain-owned. Never the mechanism
  behind it.
- `Map`: nodes and edges folded from the map events in the log, and a
  forest - every edge runs from a node to the one hanging under it, one
  edge reaches a node, and what no edge reaches heads the map. Every
  change goes through `Map::apply`, which refuses a second edge into
  one node and one that closes a cycle, so the rules live once. The
  code map is the exception, built with `Map::empty_graph`: a package
  is imported by many files, and it is never folded from the log nor
