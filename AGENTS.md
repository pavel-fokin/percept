...
| Presentation | `server` | `percept web` - serves the embedded page and the cut it reads: `GET /api/events`, this project's log filtered by the same query `events search` builds, and `GET /api/events/{id}` for one event whole. The cut is the view's, not a general query: a `tool.resulted` is never a row, so it never appears among the matches, and one whose call is in the cut comes back under `carried` for the row that folds it. Read-only for now; the review queue's write moves back with the Attention lens. |
