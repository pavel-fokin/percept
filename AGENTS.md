...
| Presentation | `cli` | `percept events publish`, `search`, `show`, `percept maps` - the log and its maps without the TUI; `ask` under `lab`. `hook <client>` records a coding client's turn from the hook JSON on stdin; `init <client>` writes the shipped schemas to `.percept/schemas` and the client's config to call the hook. Headless, a call the policy would ask about is declined unless `ask --yes`. |
