...
| Presentation | `cli` | A bare `percept` prints the start screen - how to record, then the project's maps whole - the one text a coding client's session also opens on. `percept events search`, `show`, `percept maps` - the log and its maps without the TUI; `ask` and `code`, the TUI, under `lab`. `hook <client>` records a coding client's turn from the hook JSON on stdin; `init <client>` writes the shipped schemas to `.percept/schemas` and the client's config to call the hook. Headless, a call the policy would ask about is declined unless `ask --yes`. |
