...
| Presentation | `server` | `percept web` - serves the embedded page and the cuts it reads. `GET /api/events` is this project's log filtered by the same query `events search` builds, and `GET /api/events/{id}` one event whole. The cut is the view's, not a general query: a `tool.resulted` is never a row, so it never appears among the matches, and one whose call is in the cut comes back under `carried` for the row that folds it. `GET /api/projects` is the one route that answers for more than this project - every root the log carries, with each one's maps, their headline count, and what each gained since the last session before this view opened. Every other path serves the page, which routes itself; an unknown path under `/api` is a 404. Read-only for now; the review queue's write moves back with the Attention lens. |
