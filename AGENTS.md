...
module docs of `core`, `store`, and `mapstore`.
...
- `Model`: "a reply given messages", domain-owned. Never the mechanism
  behind it.
- `Map`: nodes and edges folded from the map events in the log - a
  graph. An edge runs from a node to the one hanging under it. A node
  may take more than one edge, and an edge may close a cycle; what no
  edge reaches heads the map. A render walks from those heads, holding
...
- `Schema`: a map kind - purpose, node and edge kinds - loaded
  ...
  is the order sections print in.
...
| Presentation | `cli` | A bare `percept` prints the start screen - how to record, then the project's maps whole - the one text a coding client's session also opens on. `add`, `change`, `remove`, `show`, `search` - the log and its maps without the TUI, naming no map: a node's kind or short id finds its map through the schemas, and a write prints the level it landed on; `ask` and `code`, the TUI, under `lab`. Outside any project the root is `$HOME`, the global level: `search` there reads the whole log, and `init`, `ask`, and `code` refuse. `hook <client>` records a coding client's turn from the hook JSON on stdin; `init <client>` writes the shipped schemas to `.percept/schemas`, the global `projects` schema to `$HOME` if absent, and the client's config to call the hook. Headless, a call the policy would ask about is declined unless `ask --yes`. |
| Presentation | `server` | `percept web` - serves the embedded page and the cuts it reads. `GET /api/events` is this project's log filtered by the same query `search` builds, and `GET /api/events/{id}` one event whole. The cut is the view's, not a general query: a `tool.resulted` is never a row, so it never appears among the matches, and one whose call is in the cut comes back under `carried` for the row that folds it. `GET /api/projects` and `GET /api/maps/{id}` answer for more than this project: the first for every root the log carries, with each one's maps, their node count, and what each gained since the last session before this view opened; the second for the map `id` names, whole, folded from that project's own schemas. Every other path serves the page, which routes itself; an unknown path under `/api` is a 404. Read-only for now; the review queue's write moves back with the Attention lens. |
...
| Infrastructure | `mapstore` | Loads the schemas as `SchemaCatalog`, the `core::Schemas`: `$HOME/.percept/schemas/*.toml`, the global ones, whose maps live at `$HOME`, then the project's `.percept/schemas/*.toml`. Holds the templates `init` writes there, and folds a log-backed cognitive map (`LogMaps`, the `core::MapReader`), revises it, and gives it an external form: `encode_*` to JSON lines, `markdown` to the text `show` prints - read live, never written to a file. |
...
| Foundation | `shared` | `Id<T>`, `Timestamp`, `parse_time` - value types with no domain meaning, and the grammar of a moment a reader types (`<N>d`, `<N>h`, `<N>m`, or ISO-8601). `parse_time` answers `Option`, so each surface words its own refusal. Below the domain; depends only on `uuid`, `jiff`. |
...
The TUI, `percept code`, builds under `--features lab` and only runs
on a real terminal. `scripts/drive.py` forks a pty,
