...
- `.claude/settings.json` runs `percept hook claude-code` on session
  start and every prompt, tool use, and stop, and allows `percept maps`,
  `percept events`, and `percept start` without a permission prompt.
  `percept init claude-code` regenerates it.
