...
1. **Decompose.** Split the request into issues. Order them so each
   one builds on the last. Show the set to the user and get agreement
   before any code. An explicit request to implement a proposal
   already discussed supplies that agreement; ask only about choices
   still open that the user lives with.
