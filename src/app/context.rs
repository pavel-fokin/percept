...
fn line(event: &Event) -> Option<String> {
    // A file registration is never a message, so it has no
    // `history_message` - it still names what was read, one line, same
    // as any other event past the window.
    if let crate::core::Payload::FileRegistered { path, lines, .. } = event.payload() {
        return Some(format!(
            "{} {}: read {}",
            event.id().as_uuid(),
            event.actor().name(),
            crate::core::registration_label(path, *lines)
        ));
    }
