...
//! project's schemas from `.percept/schemas/*.toml` alone; `templates`
//! are the `decisions` and `concepts` TOML `percept init <client>`
//! copies there for a project that has none yet. A project with no
...
pub use schemas::{load as load_schemas, templates};
pub use start::start;
