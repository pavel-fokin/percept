...
fn a_session_from_another_client_at_this_path_sets_the_since_count() {
    ...
    let events = vec![session, created_at(node_added("topic", "fresh"), after)];
...
fn a_session_at_a_different_path_does_not_set_the_since_count() {
    ...
    let events = vec![session, node_added("topic", "fresh")];
...
#[test]
fn one_open_chore_shows_its_state_count() {
    let mut properties = BTreeMap::new();
    properties.insert("why".to_string(), "it matters".to_string());
    properties.insert("state".to_string(), "open".to_string());
    let events = vec![node_added_on("chores", "chore", "ship it", properties, Vec::new())];
...
fn attention_marks_a_fresh_node_as_added() {
    ...
    let since = session.created_at();
    let after = since.minus_minutes(-10).unwrap();
    let events = vec![session, created_at(node_added("topic", "fresh one"), after)];

    let text = rendered(&events, Fixture::new().path());

    assert!(text.contains("topic \"fresh one\""), "{text:?}");
...
fn attention_marks_a_node_changed_by_a_human_with_who_and_why() {
    ...
    let verdict = created_at(node_added("verdict", "old one"), earlier);
    ...
    let events = vec![session, verdict, change];
...
fn next_offers_a_read_around_for_every_attention_id() {
    ...
    assert!(text.contains("read around t1"), "{text:?}");
    assert!(text.contains("percept maps show debates --around t1"), "{text:?}");
}
...
fn next_has_no_read_line_for_a_map_with_no_headline() {
    ...
    assert!(text.contains("percept maps show debates"), "{text:?}");
    assert!(!text.contains("percept maps show chores"), "{text:?}");
}
...
fn a_deleted_file_reports_gone() {
    ...
    let mut properties = BTreeMap::new();
    properties.insert("why".to_string(), "it matters".to_string());
    let node = node_added_on("chores", "chore", "fix it", properties, vec![cited.id()]);
    let events = vec![cited, node];
...
fn a_superseded_verdict_is_still_a_headline_and_still_checked() {
    ...
    let new = node_added_citing(Actor::Human(human()), "verdict", "new answer", Vec::new());
    let edge = edge_added("replaces", &new, &old);
...
#[test]
fn resolving_an_old_topic_does_not_report_it_as_gained() {
    let session = Event::session_started(source_at("claude-code", ROOT));
    let since = session.created_at();

    let topic = created_at(
        node_added("topic", "an old topic"),
        since.minus_minutes(120).unwrap(),
    );
    let verdict = created_at(
        node_added("verdict", "a fresh verdict"),
        since.minus_minutes(-30).unwrap(),
    );
    let edge = created_at(
        edge_added("settles", &verdict, &topic),
        since.minus_minutes(-10).unwrap(),
    );
    let events = vec![session, topic, verdict, edge];

    let text = rendered(&events, Fixture::new().path());

    assert!(text.contains("+1 since last session"), "{text:?}");
    assert!(text.contains("verdict \"a fresh verdict\""), "{text:?}");
    assert!(!text.contains("an old topic"), "{text:?}");
}
