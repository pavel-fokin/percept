...
#[test]
fn a_chain_of_edges_reads_as_parent_and_children() {
    ...
    // and `a` hangs deepest.
    assert_eq!(map.parent(a).map(|node| node.id), Some(b));
    assert_eq!(map.parent(b).map(|node| node.id), Some(c));
    assert_eq!(map.parent(c).map(|node| node.id), None);
    assert_eq!(
...
fn an_unknown_kind_fails_the_fold() {
    ...
    let err = Map::fold(crate::core::testing::map_id("debates"), debates(), &[stray]).err().unwrap();
...
fn a_name_is_unique_within_its_kind_only() {
    ...
    let err = Map::fold(crate::core::testing::map_id("debates"), debates(), &events).err().unwrap();
...
fn an_edge_needs_both_ends_and_is_stated_once() {
    ...
    let err = Map::fold(crate::core::testing::map_id("debates"), debates(), &with_dangling)
        .err()
        .unwrap();
    assert!(matches!(
...
fn linked_reads_in_both_directions() {
    ...
    assert_eq!(map.parent(blocked).map(|node| node.id), Some(blocker));
    assert!(map.children(blocked).is_empty());
    assert_eq!(map.parent(blocker).map(|node| node.id), None);
}

#[test]
fn a_second_edge_into_one_node_is_refused() {
    let mut map = Map::empty(crate::core::testing::map_id("chores"), chores());
    map.apply(add_chore("a"), Actor::Human(human())).unwrap();
    map.apply(add_chore("b"), Actor::Human(human())).unwrap();
    ...
    assert!(matches!(err, MapError::SecondParent { .. }), "{err}");
}

#[test]
fn an_edge_closing_a_cycle_is_refused() {
    let mut map = Map::empty(crate::core::testing::map_id("chores"), chores());
    map.apply(add_chore("a"), Actor::Human(human())).unwrap();
    map.apply(add_chore("b"), Actor::Human(human())).unwrap();
    map.apply(
    ...
    assert!(matches!(err, MapError::EdgeWouldCycle { .. }), "{err}");
}

#[test]
fn an_edge_from_a_node_to_itself_is_refused() {
    let mut map = Map::empty(crate::core::testing::map_id("chores"), chores());
    map.apply(add_chore("a"), Actor::Human(human())).unwrap();
...
#[test]
fn a_map_built_with_empty_graph_accepts_a_second_parent_and_a_cycle() {
    let mut map = Map::empty_graph(crate::core::testing::map_id("chores"), chores());
    map.apply(add_chore("a"), Actor::Human(human())).unwrap();
    map.apply(add_chore("b"), Actor::Human(human())).unwrap();
    map.apply(add_chore("c"), Actor::Human(human())).unwrap();
    map.apply(
    ...
    .unwrap();
    map.apply(
        add_edge("blocks", node_ref("chore", "c"), node_ref("chore", "a")),
        Actor::Human(human()),
    )
    .unwrap();
...
fn select_on_an_empty_map_is_the_empty_map_not_a_missing_node() {
    ...
    let fragment = Map::empty(crate::core::testing::map_id("debates"), debates()).select(&selection).unwrap();
...
fn since_on_a_log_folded_map_is_fine() {
    ...
    assert!(Map::empty(crate::core::testing::map_id("debates"), debates()).select(&selection).is_ok());
}
